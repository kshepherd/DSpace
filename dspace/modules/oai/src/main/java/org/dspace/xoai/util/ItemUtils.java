...
public class ItemUtils
{
    ...
    public static Metadata retrieveMetadata (Item item) {
        ...
        // Create empty (anonymous) context for policy / action testing
        Context context;
        try {
            context = new Context();
        ...
        try
        {
            ...
            for (Bundle b : bs)
            {
                ...
                for (Bitstream bit : bits)
                {
                    ...
                    // Get simple flag to indicate if this bitstream is
                    // publicly readable. We could possibly populate richer
                    // policy information here

                    Boolean isPublic = AuthorizeManager.authorizeActionBoolean(context,bit,Constants.READ);
                    ...
                    bitstream.getField().add(
                            createValue("sid", bit.getSequenceID()
                                    + ""));
                    bitstream.getField().add(
                            createValue("public", String.valueOf(isPublic))
                    );
                }
            }
        }
