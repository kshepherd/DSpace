...
public class XOAI {
    ...
    private void addLincolnFields(SolrInputDocument doc, Item item) {
        String specialGroupName = Objects.toString(ConfigurationManager.getProperty("oai", "special.group.name"), "LincolnLDAP");
        Group specialGroup = null;
        try {
            specialGroup = Group.findByName(context, specialGroupName);
        } catch (SQLException e) {
            log.warn("Lincoln: Could not find group \"" + specialGroupName + "\"for Lincoln access. Only checking for public access now. Error was " + e.getMessage(), e);
        }

        Date earliestPublicAccess = null;
        Date earliestSpecialAccess = null;

        boolean anyPublicAccess = false;
        boolean anySpecialAccess = false;

        log.info("Checking access for Lincoln groups. Item id=" + item.getID());
        ...
        try {
            ...
            for (Bundle bundle : contentBundles) {
                ...
                for (Bitstream bitstream : bitstreams) {
                    List<ResourcePolicy> bitstreamPolicies = AuthorizeManager.getPoliciesActionFilter(context, bitstream, Constants.READ);
                    for (ResourcePolicy policy : bitstreamPolicies) {
                        if (policy.getGroupID() == Group.ANONYMOUS_ID) {
                            anyPublicAccess = true;
                            if (policy.getStartDate() != null && // there is a start date
                                    (policy.getEndDate() == null || policy.getEndDate().after(now)) && // there is no end date, or end date in the future
                                    (earliestPublicAccess == null || earliestPublicAccess.after(policy.getStartDate()))) { // start date is earlier than earliest access so far
                                earliestPublicAccess = policy.getStartDate();
                            }
                        } else if (specialGroup != null && isSameOrSubgroup(specialGroup, policy.getGroup())) {
                            anySpecialAccess = true;
                            if (policy.getStartDate() != null && // there is a start date
                                    (policy.getEndDate() == null || policy.getEndDate().after(now)) && // there is no end date, or end date in the future
                                    (earliestSpecialAccess == null || earliestSpecialAccess.after(policy.getStartDate()))) { // start date is earlier than earliest access so far
                                earliestSpecialAccess = policy.getStartDate();
                            }
                        }
                    }
                }
            ...
            if (anyPublicAccess) {
                doc.addField("item.lincoln.anyFileAccess_Anonymous", true);
                if (earliestPublicAccess != null) {
                    doc.addField("item.lincoln.earliestFileAccess_Anonymous", earliestPublicAccess);
                }
            }
            if (anySpecialAccess) {
                String specialGroupNameForDocField = StringUtils.replace(specialGroupName, " ", "");
                doc.addField("item.lincoln.anyFileAccess_" + specialGroupNameForDocField, true);
                if (earliestSpecialAccess != null) {
                    doc.addField("item.lincoln.earliestFileAccess_" + specialGroupNameForDocField, earliestSpecialAccess);
                }
            }
        } catch (SQLException e) {
            log.warn("Lincoln: Could not determine earliest public/Lincoln access date. Error was " + e.getMessage(), e);
        }
    }

    private boolean isSameOrSubgroup(Group potentialChild, Group potentialParent) {
        if (potentialChild == null && potentialParent == null) {
            return true;
        }
        if (potentialChild == null || potentialParent == null) {
            return false; // we have already checked that not both are null, so if one is null that's a problem
        }
        if (potentialParent.getID() == potentialChild.getID() || potentialParent.isMember(potentialChild)) {
            return true;
        }
        // recursively check whether potential subgroup is a member of actual parent or its member groups
        Group[] actualSubgroups = potentialParent.getMemberGroups();
        for (Group actualSubgroup : actualSubgroups) {
            if (isSameOrSubgroup(potentialChild, actualSubgroup)) {
                // found one
                return true;
            }
        }
        // if we made it here then there is no subgroup relationship
        return false;
    }
